...
export class BookletPurchaseService {
  ...
  async createOfflineBooklet(obj) {
    const resp = await this.http.post<any>(this.httpUrl + '/insertOfflineBoolet', obj).toPromise().then(res => {
      return res;
    });
    return resp;
  }

  async gatAllUsedPartyID(obj) {
    const resp = this.http.get<any>(this.httpUrl + '/getUsedPartyID' +obj).toPromise().then(res => {
      return res
    });
    return resp
  }
  async getPartyMobile_used(obj) {
    const resp = this.http.get<any>(this.httpUrl + '/getPartyMobile_used' +obj).toPromise().then(res => {
      return res
    });
    return resp
  }
