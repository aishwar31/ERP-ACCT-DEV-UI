...
import { EstablishmentService } from '../../services/hrms/service/establishment.service';
...
export class TaskComponent implements OnInit {
  ...
  async ngOnInit() {
   ...
   // await this.ebillPrintService.processebill({doc_local_no:8,node_cd:27},this.erpUser)
  ...
  async rejectnewrowapproval(element,obj) {
    this.spinner.show()
    var obb = Object()
  ...
  async Approvedapproval() {
    ...
    if (this.forwarded_user_id != null || this.ApprovalUserService.vendorflag == true) {
      ...
      if (resp['error'] == false) {
        ...
        // await this.updatestatus(this.tender_apprdata['id'])
        this.spinner.hide();
        if(this.ApprovalUserService.conditionflag == false) swal.fire("Success", "Approval Forwarded Successfully!!", 'success');
...
if (resp['error'] == false) {
  ...
  async underapprovalapproval(element) {
    this.spinner.show()
  ...
  /*   if(this.tender_apprdata['doc_type'] == 'EBILL') {
      await this.ebillPrintService.processebill(this.tender_apprdata,this.erpUser)
      return
    } */
