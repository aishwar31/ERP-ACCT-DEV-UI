...
export class ActivityHierComponent implements OnInit {
  ...
  async save() {
    ...
    var resp = await this.hierarchyService.createActivityHierarchy(this.obj);
    if (resp['error'] == false) {
      this.spinner.hide();
  ...
  async update() {
    ...
    this.obj['b_acct_id'] = this.b_acct_id;
    this.obj['table_name'] = 'activity_hier';
console.log(this.oldUpdateObject)
    if (this.obj['lvl' + this.updateLevel + "_type"] == 'text') {
      for (let i = 1; i < 8; i++) {
        ...
        if (this.obj['lvl' + i + '_type'] != undefined) {
          if (this.obj['lvl' + i + '_type'] != '') {
            ...
            if (this.unique.includes(this.obj['lvl' + i + '_cd']) && this.oldUpdateObject['leaf_cd']!=this.obj['lvl' + i + '_cd']) {
              swal.fire('Error', "Duplicate value " + this.obj['lvl' + i + '_cd']);
              console.log('inside')
              this.spinner.hide();
      ...
      if (this.unique_leaf.includes(this.obj['leaf_cd']) && this.oldUpdateObject['leaf_cd']!=this.obj['leaf_cd']) {
        swal.fire('Error', "Duplicate leaf " + this.obj['leaf_cd']);
        this.spinner.hide();
    ...
    var resp = await this.hierarchyService.updateActivityHierarchy(this.obj);
    if (resp['error'] == false) {
      this.flag = 1
      this.spinner.hide();
      if (this.flag == 1) {
  ...
  async delete(element) {
    ...
    obj['table_name'] = 'activity_hier';
    this.spinner.show()
    var resp = await this.hierarchyService.deleteActivityHierarchy(obj);
    if (resp['error'] == false) {
      await this.getAllHier()
      this.spinner.hide();
  ...
  async SubmitListHierforMove(){
    ...
    var resp = await this.hierarchyService.moveActivityHierarchy(this.obj);
    console.log(resp)
    if (resp['error'] == false) {
      this.spinner.hide();
  ...
  async open_update(element) {
    this.oldUpdateObject=JSON.parse(JSON.stringify(element))
