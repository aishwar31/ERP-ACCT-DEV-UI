...
export class BillComponent implements OnInit {
  ...
  constructor(private userAdd: UserAddService, private settingService: SettingService, private ruleProcessService: RuleProcessService,
    private budgetService: BudgetService, private EventsService: EventsService, public mainService: MainService, private ledgerService: LedgerService, private ruleService: RuleService, private billService: BillService, private router: Router, private spinner: NgxSpinnerService, private snackBar: MatSnackBar, private hierarchyService: HierarchyService) { }
  erpUser;
  b_acct_id;
    ...
    if (this.type == 'budget') {
      for (let i = 0; i < this.allBudget.length; i++) {
        if (this.allBudget[i]['code'] == this.Chartobj['leaf_cd']) {
          ...
          this.selectObj['bud_obj'] = this.Chartobj;
          console.log(this.allBudget[i]);
      ...
      for (let i = 0; i < this.allActivity.length; i++) {
        if (this.allActivity[i]['code'] == this.Chartobj['leaf_cd']) {
          ...
          this.selectObj['act_obj'] = this.Chartobj;
      ...
      for (let i = 0; i < this.allProject.length; i++) {
        if (this.allProject[i]['code'] == this.Chartobj['leaf_cd']) {
          ...
          this.selectObj['proj_obj'] = this.Chartobj;
          console.log(this.allProject[i]);
          console.log(this.Chartobj);
      ...
      for (let i = 0; i < this.allProduct.length; i++) {
        if (this.allProduct[i]['code'] == this.Chartobj['leaf_cd']) {
          ...
          this.selectObj['prod_obj'] = this.Chartobj;
  ...
  async checkBudgetAmount() {
    ...
    for (let i = 0; i < this.AllBillRow.length; i++) {
      ...
      for (let j = 0; j < this.allAllocationArr.length; j++) {
        console.log(budArr.includes((this.allAllocationArr[j]['bud_cd']).toString())); 
        console.log(actArr.includes((this.allAllocationArr[j]['act_cd']).toString()));     
        console.log(projArr.includes((this.allAllocationArr[j]['proj_cd']).toString())); 
        console.log(prodArr.includes((this.allAllocationArr[j]['prod_cd']).toString()));
        if (budArr.includes(this.allAllocationArr[j]['bud_cd'].toString()) && actArr.includes(this.allAllocationArr[j]['act_cd'].toString())
          && projArr.includes(this.allAllocationArr[j]['proj_cd'].toString()) && prodArr.includes(this.allAllocationArr[j]['prod_cd'].toString())) {
            console.log("Inside");
