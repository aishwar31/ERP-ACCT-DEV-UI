    ...
    for (let i = 0; i < bpid.length; i++) {
      for (let j = 0; j < this.bp_data.length; j++) {
        if (this.bp_data[j]['id'] == bpid[i]) {
          ...
          console.log(tt)
          console.log(this.ChartOfAccountToAccountObj)
bankobj=Object.assign({},tt)
          bank_acct_no = this.ChartOfAccountToAccountObj[tt['org_bank_acct_no']]
        }
      }
    }
    console.log(this.bankDtlObj,bank_acct_no)
    let bank = bankobj;
    console.log(bank)

    console.log(bank['bank_code'])
    console.log(this.mainService.codeValueShowObj['ACC0006'][bank['bank_code']])
