...
export class WorkService {
  ...
  async  getWorkList(obj) {
    const resp = await this.http.get<any>(this.httpUrl+'/getworkInfo' + JSON.stringify(obj)).toPromise().then(res => {
      return res;
    });
    return resp;
  }
  ...
  async deleteWork(obj){
    const res = await this.http.put<any>(this.httpUrl + '/deleteWork' , obj).toPromise().then(res => {
      return res;
    });
    return res;
  }
