...
export class ChallanComponent implements OnInit {
  ...
  dataSource1;
  activeChallanList = true;
  underProcessedChallanList = false;
  createChallans: any;
  updateChallans: boolean;
  ...
  async payment_received() {
    ...
    if (paid.length != 0 || processed.length != 0) {
      ...
      let chlnObj = new Object();
      chlnObj['status'] = 'UNDERPROCESS'
      if (resp['error'] == false) {
        this.spinner.hide();
        this.getChallanInfo(chlnObj);
        swal.fire("Success", "...Challan PAID Successfuly!", 'success');
      } else {
        this.spinner.hide();
        swal.fire("Error", "...Error while Challan Payment!!", 'error');
      }
    }
  }
  ...
  async open_update(element) {
    ...
    console.log(this.challanRows)

    this.check()
    console.log("check");
    // $('.nav-tabs a[href="#tab-3"]').tab('show')
    await this.update_chln();

  }

  allChallan = [];
  async getChallanInfo(data) {
    var obj = new Object
  ...
  async open_list_chln() {
    let obj = {}
    this.activeChallanList = true;
    this.underProcessedChallanList = false;
    this.createChallans = false;
    this.updateChallans = false;
    obj['status'] = 'ACTIVE'
    await this.getChallanInfo(obj);
  }

  async underProcessed_chln() {
    let obj = {}
    this.activeChallanList = false;
    this.underProcessedChallanList = true;
    this.createChallans = false;
    this.updateChallans = false;
    obj['status'] = 'UNDERPROCESS'
    await this.getChallanInfo(obj);
  }
